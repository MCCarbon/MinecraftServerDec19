...
public class ChunkProviderTheEnd implements IChunkProvider {
	...
	public ChunkProviderTheEnd(World world, long seed) {
		this.world = world;
		...
		for (int var8 = -12; var8 <= 12; ++var8) {
			for (int var9 = -12; var9 <= 12; ++var9) {
				...
				if ((((var10 * var10) + (var11 * var11)) > 4096) && (this.whatIsThis.a(var10, var11) < -0.8999999761581421D)) {
					float var12 = (((MathHelper.abs(var10) * 3439) + (MathHelper.abs(var11) * 147)) % 13) + 9;
					var5 = var3 - (var8 * 2);
					var6 = var4 - (var9 * 2);
	...
	public void getChunkAt(IChunkProvider provider, int chunkX, int chunkZ) {
		...
		this.world.getBiome(position.add(16, 0, 16)).a(this.world, this.world.random, position);
		if (((chunkX * chunkX) + (chunkZ * chunkZ)) > 4096) {
			if ((this.a(chunkX, chunkZ, 1, 1) < -20.0F) && (this.random.nextInt(14) == 0)) {
				this.gatewayPlatformGenerator.generate(this.world, this.random, position.add(this.random.nextInt(16) + 8, 55 + this.random.nextInt(16), this.random.nextInt(16) + 8));
			...
			for (int i = 0; i < rndi; ++i) {
				...
				if (rndY > 0) {
					...
					if (this.world.isEmpty(position.add(rndX, rRndY + 1, rndZ)) && (this.world.getType(position.add(rndX, rRndY, rndZ)).getBlock() == Blocks.END_STONE)) {
						BlockChorusFlower.a(this.world, position.add(rndX, rRndY + 1, rndZ), this.random, 8);
					}
				}
			}
		}
